...
static NSString *getSubdir(NSString *rootPath, NSString *relativePath) {
...
+ (instancetype) v1 {
    ...
    inst->_root = root;
    inst->_sessions = getSubdir(root, @"sessions");
    inst->_breadcrumbs = getSubdir(root, @"breadcrumbs");
    inst->_kscrashReports = getSubdir(root, @"KSCrashReports");
    inst->_kvStore = getSubdir(root, @"kvstore");
