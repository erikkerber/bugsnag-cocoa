...
- (void)testMigrateFiles {
    ...
    // Make sure the client still loads and files are still as we expect.
    BugsnagConfiguration *configuration = [[BugsnagConfiguration alloc] initWithApiKey:DUMMY_APIKEY_32CHAR_1];
    (void)[[BugsnagClient alloc] initWithConfiguration:configuration];
    [self postMigrateCheck];

    // One more idempotency test.
    [BSGStorageMigratorV0V1 migrate];
    [self postMigrateCheck];
}
